...
#
# Perform backups with Oracle RMAN 9R2.
...
#
RMAN_CONF="/usr/home/oracle/scripts/${NAME}/${NAME}.conf"
...
#
f_print_usage() {
	printf "\nUsage: ${NAME}.sh -t {daily} -n <database name to backup>\n"

	printf "\nWhere:\t-s: Database name to backup.\n"
	printf "\t-t: Type of backup: only daily in 9i version."
	printf "\nExample: ${NAME}.sh -t daily -n CNQ\n\n"
...
if [ "x${RMAN_CONF}" == "x" ]; then
	echo "ERROR: Please set the RMAN_CONF variable in ${NAME}."
	exit 1

elif [ ! -f "${RMAN_CONF}" ]; then
	echo "ERROR: Could not find the configuration file ${RMAN_CONF}."
	exit 1

elif [ -z "${RMAN_CONF}" ]; then

	echo "ERROR: Configuration file ${RMAN_CONF} is empty."
	exit 1
fi
...
while getopts "t:n:" OPTIONS; do
	...
	case ${OPTIONS} in
		...
		t)	BACKUP_TYPE="${OPTARG}"
			...
			echo ${BACKUP_TYPE} | egrep -i '(daily)' 2>&1 >/dev/null

			if [ $? -ne 0 ]; then
				echo "ERROR: Illegal backup type value (${BACKUP_TYPE}). Bailing out."
				echo "NOTE: Backup type must be one of: archive, catalog, daily or prod."
				exit 1
			else
...
if [ "x${LOGDIR}" == "x" ]; then
	echo "ERROR: Please configure LOGDIR in ${RMAN_CONF}."
	exit 1
...
if [ "x${PASSWD_FILE}" = "x" ]; then
	echo "ERROR: Please set PASSWD_FILE in ${RMAN_CONF}. Bailing out." | tee -a ${LOGFILE}
	exit 1
...
# We only have the daily case here, but the point is that the script is ready for another type of backup.

case ${BACKUP_TYPE} in

	# Daily backup of the dev, test and pre-production databases: ntsdev, ntstest and ntspp.
	daily)
		if [ "x${DAILY}" == "x" ]; then
			echo "ERROR: Please set DAILY to the daily stored script name in the ${RMAN_CONF} file." | tee -a ${LOGFILE}
			exit 1
		fi

		echo "run { execute script ${DAILY}; }" | rman target sys/${SYS_PASSWD}@${DB_NAME} catalog rman/${RMAN_PASSWD}@${RCVCAT} log ${LOGFILE} append
	;;
...
# Read the log file to generate an email.
